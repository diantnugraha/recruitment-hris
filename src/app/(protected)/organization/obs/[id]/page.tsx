...
export default function OBSDetailPage() {
  ...
  const fetchDetail = async () => {
    setIsLoading(true);
    const response = await obsService.getById(id);
    if (response.success && response.data) {
      ...
      showToast.fetchError("organization", response.message);
    }
    setIsLoading(false);
  };
  ...
  const handleSave = async () => {
    ...
    const response = await obsService.update(id, {
      name: formData.name,
  ...
  const handleDelete = async () => {
    setIsSubmitting(true);
    const response = await obsService.delete(id);
    if (response.success) {
      showToast.deleted("Organization");
      router.push("/organization/obs");
    } else {
          ...
          {isEditing ? (
            ...
            >
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                  <Label style={{ fontSize: "0.875rem", fontWeight: 500, color: "var(--hsd-ui-color-gray-700)" }}>Cluster</Label>
                  <Input value={formData.cluster} onChange={(e) => setFormData({ ...formData, cluster: e.target.value })} placeholder="Enter cluster" />
                </div>
              </div>
              ...
              >
                <div className="p-6">
                  <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
                    ...
                    >
                      <Network style={{ width: "36px", height: "36px", color: "var(--hsd-ui-color-navy-500)" }} />
                    </div>
                    <div className="flex-1 min-w-0 sm:pt-2">
                      ...
                      <div className="flex items-center gap-2 mt-2 flex-wrap">
                        {org.cluster && <TuvBadge text={org.cluster} variant="info" size="sm" border />}
                      </div>
                      <div className="mt-5 pt-4 grid grid-cols-2 gap-x-8 gap-y-3" style={{ borderTop: "1px solid rgba(120, 134, 127, 0.15)" }}>
                        ...
                        <DetailItem label="Cluster" value={org.cluster || ""} />
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <SectionCard title="Description" icon={FileText}>
                <p
                  style={{
                    fontSize: "0.875rem",
                    fontWeight: 400,
                    color: org.description ? "var(--hsd-ui-color-gray-700)" : "var(--hsd-ui-color-gray-400)",
                    margin: 0,
                    whiteSpace: "pre-wrap",
                    fontStyle: org.description ? "normal" : "italic",
                  }}
                >
                  {org.description || "No Data"}
                </p>
              </SectionCard>
            </>
          )}
